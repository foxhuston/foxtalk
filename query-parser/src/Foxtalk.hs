...
queryValueToTupleEntry (VVarIntro _ _)      = "TupleValue::query()"
...
queryValuesToPosition = posn 0
  where
    ...
    posn n (VVarIntro t _:xs)       = VVarIntro t n : posn (n + 1) xs
    posn n (VVarBinding _:xs)       = VVarBinding n : posn (n + 1) xs
    posn n (VVarIntroLit t _ l:xs)  = VVarIntroLit t n l : posn (n + 1) xs
