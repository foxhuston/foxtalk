...
protected:
  ...
  void handle(const std::vector<Tuple> &queryResults) override {
    ...
    // auto need_to_create_new_swapchain = true;
    // if (last_out_of_date != queryResults.end()) {
    ...
    //     debug << "[SwapchainHandler] Need to create a new swapchain because "
    ...
    debug << "Logical device ptr is " << logical_device << end;
    ...
    auto width = surface_tuple->at<uint64_t>(8).value();
    auto height = surface_tuple->at<uint64_t>(10).value();
    ...
    if (minImageCount < surface_caps.minImageCount) {
      if (surface_caps.minImageCount > MAX_NUM_IMAGES) {
        ...
        err << "surface_caps.minImageCount is too large (is: "
            << surface_caps.minImageCount << ", max: " << MAX_NUM_IMAGES << end;
    ...
    if (swapchainCreationResult == VK_ERROR_NATIVE_WINDOW_IN_USE_KHR &&
      ...
      debug << "Tried to create a new swapchain, but the surface hasn't been "
               "recreated yet... so, just resserting the old one for now"
            << end;
      reuse_swapchain();
      return;
    }
    if (swapchainCreationResult != VK_SUCCESS) {
      err << "Error creating swapchain! Result: " << swapchainCreationResult
          << end;
      return;
    }
    ...
    for (auto *image : swapchain_images) {
      ...
      imageNouns.emplace_back("at index");
      imageNouns.emplace_back(j);
      j++;
    }
  ...
  void reuse_swapchain() {
    ...
    debug << "Reusing swapchain with version "
          << last_created_swapchain_nouns[4].get<double_t>().value() << end;
