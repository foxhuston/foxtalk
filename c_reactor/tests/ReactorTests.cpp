...
struct TestHandler : Handler {
    Tuple *get_query() const override { return Tuple::mk(mkSymbol("lexi"), mkSymbol("is a"), mkSymbol("husky")); }
...
struct MaxHandler : Handler {
    Tuple *get_query() const override { return Tuple::mk(mkQuery(), mkSymbol("has size"), mkQuery()); }
    ...
    void handle_results(ReactorVec<const Tuple *>::type tv, std::function<void(const Tuple *)> claim) const override {
        ...
        if (max_subj != nullptr) { claim(Tuple::mk(max_subj, mkSymbol("is"), mkSymbol("the biggest!"))); }
    }
};
...
BOOST_AUTO_TEST_CASE(ReactorWillRemoveGeneratedTuplesTest) {
    ...
    auto results = reactor.query(mkQuery(), mkSymbol("is"), mkSymbol("the biggest!"));

    std::cout << "Results contains \"ball d is the biggest?\"" << std::endl;
    BOOST_ASSERT(results.contains(Tuple::mk(mkSymbol("ball d"), mkSymbol("is"), mkSymbol("the biggest!"))));
    ...
    auto post_delete_results = reactor.query(mkQuery(), mkSymbol("is"), mkSymbol("the biggest!"));

    std::cout << "Post delete results:" << std::endl;
    for (auto t: post_delete_results) { std::cout << "   " << *t << std::endl; }

    auto inner_db_results =
            reactor.get_db().get_tuples().contains(Tuple::mk(mkSymbol("ball d"), mkSymbol("has size"), mkU64(1000)));
    ...
    auto contains_ball_d_is_biggest =
            post_delete_results.contains(Tuple::mk(mkSymbol("ball d"), mkSymbol("is"), mkSymbol("the biggest!")));

    BOOST_ASSERT(!contains_ball_d_is_biggest);

    std::cout << "Post delete results contains \"ball c is the biggest?\"" << std::endl;

    auto contains_ball_c_is_biggest =
            post_delete_results.contains(Tuple::mk(mkSymbol("ball c"), mkSymbol("is"), mkSymbol("the biggest!")));
...
BOOST_AUTO_TEST_CASE(ReactorCallsExternalHandler) {
    ...
    auto expected = Tuple::mk(mkSymbol("lexi"), mkSymbol("is"), mkSymbol("super cool"));
    BOOST_ASSERT(tuples.contains(expected));

    auto expected2 = Tuple::mk(mkSymbol("lexi"), mkSymbol("is an"), mkSymbol("awesome puppy"));
...
BOOST_AUTO_TEST_CASE(ReactorGeneratedTuplesAreTransitivelyRemoved) {
    ...
    auto expectedB = Tuple::mk(mkSymbol("fox"), mkSymbol("loves"), mkSymbol("lexi"));
...
BOOST_AUTO_TEST_CASE(ReactorGeneratedTuplesAreTransitivelyRemovedByHandlerRemoval) {
    ...
    auto originalClaim = Tuple::mk(mkSymbol("lexi"), mkSymbol("is a"), mkSymbol("husky"));
