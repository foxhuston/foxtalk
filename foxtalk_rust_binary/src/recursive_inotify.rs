...
pub trait FileWatcherHandlers where Self: Send + Sync + Debug {
        ...
        thread::spawn(move || {
            ...
            for entry in sub_directories {
                ...
                if entry.file_type().unwrap().is_dir() {
                    ...
                    watcher.child_watch_descriptors.write().unwrap().insert(new_watcher.base_path.clone(), new_watcher.clone());
            ...
            loop {
                ...
                for event in events {
                    ...
                    let full_path_to_file = base_path.clone() + event.name.unwrap().to_str().unwrap();
                    if event.mask == EventMask::CREATE | EventMask::ISDIR {
                        ...
                        watcher.child_watch_descriptors.write().unwrap().insert(full_path_to_file.clone(), new_watcher);
                        continue;
                    }
                    ...
                    if event.mask == EventMask::DELETE {
                        ...
                        watcher.user_handlers.on_delete(full_path_to_file.clone(), file_name.clone(), extension.clone());
                        continue;
                    }
                    if event.mask == EventMask::CLOSE_WRITE {
                        println!("Writing to file {:?}", full_path_to_file);
                        watcher.user_handlers.on_create(full_path_to_file.clone(), file_name.clone(), extension.clone());
                        continue;
                    }
                    ...
                    println!("Waning: Found unhandled event {:?} for {:?}. Handling via user code if possible...", event.mask, full_path_to_file);
